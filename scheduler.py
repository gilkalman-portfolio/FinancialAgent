...
def _insider_cluster_scan_enabled(cfg: dict) -> bool:
    """Same normalization convention as _event_study_enabled() above — a
    missing "insider_cluster_scan" key defaults to enabled."""
    ic_cfg = cfg.get("insider_cluster_scan", True)
    return (ic_cfg is True or ic_cfg == "true" or
            (isinstance(ic_cfg, dict) and ic_cfg.get("enabled", True)))


def run_insider_cluster_scan():
    """Daily, DB-only (no Telegram) — free forward-validation data capture
    for the QuantConnect insider-cluster-buying research lead (CLAUDE.md /
    project_quantconnect_cross_check memory: sub-$2B, 2+ distinct insiders
    buying within 72h, DSR~59-65% out-of-sample). Logs a forward_signals
    WATCH row per detected cluster via src.insider_cluster_scanner — never
    places a trade, mirrors _record_watch_signals()'s data-capture-only
    contract for the News-Catalyst Event-Study Measurement. A failure here
    must never affect any other job.

    Runs in the morning and processes YESTERDAY's filing date, not today's —
    SEC's daily-index file for a given date is generated by their own
    end-of-day batch process and isn't guaranteed complete until well after
    the US close, so scanning "today" right after market close risks reading
    a partial index. Running the morning after and reading the prior
    calendar day sidesteps that race entirely; the 72h cluster window means
    a one-day processing lag doesn't lose any cross-day cluster detection."""
    if not _is_trading_day():
        logger.info("run_insider_cluster_scan: skipping — weekend")
        return
    cfg = load_config()
    if not _insider_cluster_scan_enabled(cfg):
        return
    try:
        init_db()
        from datetime import date, timedelta
        from src.insider_cluster_scanner import scan_insider_clusters
        ic_cfg = cfg.get("insider_cluster_scan", {})
        if not isinstance(ic_cfg, dict):
            ic_cfg = {}
        stats = scan_insider_clusters(
            day=date.today() - timedelta(days=1),
            max_market_cap=ic_cfg.get("max_market_cap", 2_000_000_000),
            min_price=ic_cfg.get("min_price", 1.0),
            min_dollar_volume=ic_cfg.get("min_dollar_volume", 300_000),
            cluster_window_hours=ic_cfg.get("cluster_window_hours", 72),
            min_distinct_insiders=ic_cfg.get("min_distinct_insiders", 2),
            max_workers=ic_cfg.get("max_workers", 10),
        )
        _log(f"Insider cluster scan: {stats}")
    except Exception as e:
        logger.error(f"Insider cluster scan failed: {e}")
...
def _main_body():
    ...
    # Insider Cluster Scan — free forward-validation data capture for the
    # QuantConnect research lead (see CLAUDE.md). Runs in the morning and
    # processes YESTERDAY's SEC daily index (see run_insider_cluster_scan's
    # docstring for why — avoids a race with SEC's own EOD index generation).
    ic_cfg = cfg.get("insider_cluster_scan", {})
    insider_cluster_scan_time = ic_cfg.get("scan_time", "08:10") if isinstance(ic_cfg, dict) else "08:10"
    schedule.every().day.at(insider_cluster_scan_time).do(run_insider_cluster_scan)
    logger.info(f"Insider cluster scan at {insider_cluster_scan_time}")
