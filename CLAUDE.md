...
- [ ] Deploy the 2026-08-25 `edgar_fcf._FACTS_CACHE` LRU-cap + `gc.collect()` fixes to the live `scheduler.py` process (restart required to pick up the code) and confirm memory stays bounded across a real multi-hour/full-day run — the fixes were live-verified at full production scale (2,463 tickers) in isolation, but not yet against the actual long-running process from the incident; see Incident Archive
...
**Not yet validated:** the live `scheduler.py` process from this incident (PID still running pre-fix code as of this writing — informational-only threads, no urgency to restart mid-session) hasn't itself been restarted onto the fixed code, and neither fix has been observed across a full day's real cache accumulation across every EDGAR-touching path (DCF, revenue CAGR, interest coverage, current ratio, EPS YoY fallback all share `_FACTS_CACHE`). If memory still climbs after both fixes are deployed, that's the next signal to chase.
