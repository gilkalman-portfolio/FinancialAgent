...
| `auto_watchlist_agent.py` | Auto-adds squeeze/catalyst/momentum/supertrend candidates to watchlist with Telegram summary. **`alert_score` uses `AUTO_WL_SCORE_ENTRY` (70) from hysteresis.py** — consistent with all other auto-watchlist entry thresholds (was 60 from config). **Capacity rotation** (added 2026-08-14, `_evict_for_capacity()`): when `watchlist_policy.max_items_total` is reached, a new candidate no longer gets silently dropped — the weakest eligible AUTO-added incumbent (3-day avg score, must have cleared `AUTO_WL_MIN_HOLD_DAYS`) is evicted to make room, same "replace weakest with better" logic as `scheduler.run_weekly_rotation()` but triggered on demand instead of weekly. Scored candidates (squeeze/catalyst/momentum) must strictly beat the weakest incumbent's score; unscored candidates (supertrend — no composite gate by design) can only claim a slot whose incumbent has already fallen below `AUTO_WL_SCORE_EXIT` (40), so the least-vetted source can't bump a ticker still earning its place. Root cause: `max_items_total` was a hard block with no eviction — on 2026-08-14 the watchlist was pinned at 30/30 and every source (momentum found 192–356 hits across several cycles) added 0. |
...
| `supertrend.py` | Supertrend calculation (ATR-based, Wilder EMA) — used by `ibkr_worker.py` and `price_alert_monitor.py`. **`scan_supertrend_universe()`** (added 2026-08-14) batch-downloads daily OHLCV (yfinance, ~0.2s/ticker) across the full scan universe and returns every ticker with a fresh bullish flip (`bars_ago==1`), with **no composite-score gate** — mirrors a bare TradingView `alertcondition(buySignal)`, closing the coverage gap left by `monitoring_queue.py`'s `SCANNER_MIN_SCORE=65` filter (a technically strong flip on a middling-score ticker was previously never monitored at all). |
...
| Supertrend Universe Monitor | every 30 min (thread), market hours only | `_supertrend_universe_monitor_thread()` (added 2026-08-14) — scans `supertrend_universe_indices` (default Russell 2000 + S&P 500) via `scan_supertrend_universe()`, auto-adds **every** fresh bullish flip via `auto_watchlist_agent` (source=`"supertrend"`) with no score gate — only sanity floors (price ≥ `$5`, optional liquidity). Config: `supertrend_universe_enabled`/`_interval_minutes`/`_indices` + `auto_watchlist.sources.supertrend`. Same 24h dedup cooldown as other auto-watchlist sources, so a flip that stays fresh across multiple 30-min cycles only fires once. |
