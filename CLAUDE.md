...
- [ ] `catalyst_event_study.py` needs a Newey-West / block-bootstrap correction for overlapping same-ticker forward-return windows (distinct from the cross-*ticker* date-clustering already implemented) — found by the 2026-08-26 IdeaDistill design review, not yet fixed. See Incident Archive.
- [ ] `catalyst_event_study.py`'s no-news control group isn't liquidity-matched to the news-accompanied group — a confound risk flagged by the same review, not yet fixed. Needs a matched-pairs design or an ADV covariate before the direction comparison is trustworthy. See Incident Archive.
...
- [ ] `tests/test_new_fixes.py`'s hand-rolled `forward_signals` CREATE TABLE (a second copy of the schema, not derived from `database.py::_migrate()`) will break again on the next new column — worth replacing with a real `init_db()` call at some point, not urgent.
