...
- **Tests:** `python -m pytest tests/ --ignore=tests/test_new_apis.py --ignore=tests/test_ibkr_connection.py --ignore=tests/test_ibkr_worker_once.py` → **855 passed, 1 failed** (2026-08-30; skips seen on some runs are environment-only — `ib_async` needs `.venv313`, one test needs live `.env` credentials). The 1 failure (`test_news_fetcher_massive.py::TestGetTickerNewsWiresMassive::test_massive_included_when_key_present`) is pre-existing and unrelated to `insider_cluster_scanner.py` — confirmed via `git stash` that it fails identically on the prior commit too; not investigated further this session. This number moves every sprint — re-run it rather than trusting this line for long. The old "5 pre-existing failures in test_pnl_digest_fixes.py" were test rot (helpers seeded with literal dates against a relative-date filter), fixed by using relative dates. **Never seed a time-filtered query with a literal date.**
...
- [ ] `run_scan()` (the scheduler's daily full-universe scan) is not parallelized, unlike `scan_watchlist()`/`scan_portfolio()` — a long-running scan can block every job scheduled after it in the single `schedule`-library thread (observed 2026-08-26: 15:00 run still executing 72+ min later, causing the 15:50 Premarket Gap Alert to silently miss its window). A worktree named `scheduler-worker-load-issue-399d18` may already be addressing this — check there first. See Incident Archive, 2026-08-26.
- [ ] Diagnose the 5 same-day `returncode=1` scheduler crashes on 2026-08-26 (11:34–14:29) — no traceback available (stderr devnulled by watchdog, see [[project_scheduler_logging_gap_2026_08_17]]). Consider capturing stderr to a rotating file as a lower-risk fix than piping it back through loguru.
